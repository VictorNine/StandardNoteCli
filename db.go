...
func (db *database) getItem(uuid string) (sf.Item, error) {
	var item sf.Item
	err := db.conn.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte("Notes"))
		v := b.Get([]byte(uuid))
...
func (db *database) newUnsyncedItem(item *sf.Item) error {
	bytes, err := json.Marshal(item)
	if err != nil {
		return err
	}
...
func (db *database) getUnsyncedItems() ([]sf.Item, error) {
	items := make([]sf.Item, 0)
...
func (db *database) deletetUnsyncedItem(uuid string) error {
	err := db.conn.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte("Unsynced"))
		err := b.Delete([]byte(uuid))
		return err
